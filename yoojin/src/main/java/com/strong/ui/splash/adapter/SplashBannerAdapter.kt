 ...
 */
class SplashBannerAdapter(datas: MutableList<SplashBean.SplashData>?) :
    BannerAdapter<SplashBean.SplashData, ImageHolder>(datas) {

    override fun onCreateHolder(parent: ViewGroup?, viewType: Int): ImageHolder {
        val view = LayoutInflater.from(parent?.context)
            .inflate(R.layout.layout_splash_banner, parent, false);
        return ImageHolder(view)
    }

    override fun onBindView(
        holder: ImageHolder,
        data: SplashBean.SplashData,
        position: Int,
        size: Int
    ) {
